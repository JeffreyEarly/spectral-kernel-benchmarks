...
Accelerate thread-limit candidates run in isolated processes with `VECLIB_MAXIMUM_THREADS` set before program startup. The recorded value is a requested process limit, not a claim about the number of workers Accelerate actually schedules. The bounded common, grouped, and outer-scheduling screens use the historical $256^2$, $N_z=65$, fields $=3$ and $512^2$, $N_z=129$, fields $=3$ cases. Fields 1/4, $N_z=257$, third-party grouped APIs, blocking, and packing-plus-GEMM remain outside these increments.
