...
Correctness has three explicit levels. The compact and full-control algorithms must agree with each other within `1e-12` maximum scale-normalized and relative L2 error. Each must agree with the independently generated compiled-WVM oracle within `2e-12`, accounting for the different internally threaded WVM FFT graph and outer-sharded benchmark FFT graph. MATLAB and compiled WVM must pass the separate `1e-10` cross-implementation audit. The `2e-12` oracle ceiling was introduced after the first deep-vertical diagnostic showed that the full production-layout control itself reached `1.03e-12`, while L2 remained `2.34e-13` and compact-versus-control intermediates remained below `5e-16`; that diagnostic is not reference evidence.
