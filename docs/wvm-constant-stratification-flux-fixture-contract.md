...
Correctness has three explicit levels. Both compact-versus-full-control algorithm equivalence and each algorithm's conformance to the compiled-WVM oracle use a dual norm: maximum scale-normalized error at most `2e-12` and relative L2 error at most `1e-12`. The distinct maximum ceiling accounts for a global extreme over roughly 46 million deep-workload outputs, while the original L2 standard remains unchanged. MATLAB and compiled WVM must pass the separate `1e-10` cross-implementation audit. The dual norm was finalized after discarded deep-vertical diagnostics showed maximum errors of `1.04e-12` for compact versus control and `1.03e-12` for the full production-layout control versus the oracle, while their relative L2 errors were `2.03e-13` and `2.34e-13` and compact-versus-control intermediates remained below `5e-16`. Those diagnostics are not reference evidence.

Reference evidence may advance the compact specialization only when the geometric complete-call ratio is at most `0.90`, no workload ratio exceeds `1.03`, the empirical stratified interval excludes a tie, all three correctness levels pass, the allocation and placement contracts pass, and one tuple is used across all four workloads. Any conclusion is limited to constant stratification and the tested Mac; it does not imply a general-stratification or general-Mac default.
