...
The exporter evaluates `nonlinearFlux` through both backends and refuses the fixture unless their maximum scale-normalized and relative-L2 errors are at most `1e-12`. It exports only physical configuration, logical horizontal mode keys, the three input coefficient arrays, and the three expected flux arrays. It deliberately does not export intermediate velocity, derivative, phase, scale, or projection values. `skbench` must reconstruct those independently from audited WVM formulas.

The v1 preparer accepts only a clean WVM checkout at commit `6ad254fb9756ac918bb72e036020d004879df1f2`, a clean benchmark generator checkout, the audited source hashes, an identity-validated compiled WVM module, exact payload hashes and dimensions, and a complete radial two-thirds mode set. It reorders payloads from WVM's physical radial order into `skbench`'s logical radial order outside timing.
