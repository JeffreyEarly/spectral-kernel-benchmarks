...
The preparer streams large payloads while rejecting a dirty or non-authoritative WVM export, duplicate or unsafe payload paths, unexpected payloads, malformed metadata, byte-count or SHA-256 mismatches, incorrect mode keys or WVM group maps, normalization differences, and any change to the field/operator or oracle contract. It refuses to overwrite an existing prepared file. The C++ loader independently streams the prepared binary and checks its version, byte order, authoritative marker, dimensions, retained mode-key set, WVM group map and diagnostic keys, family map, finite values, DC reality, and absence of trailing bytes. It then reorders inputs and the oracle by `(k,l)` outside timing; floating radial ties need not have the same physical order in WVM and `skbench`.

The prepared operator payload retains the smaller WVM floating-group matrix set. Canonical mode ordering records a segment-to-source-matrix map instead of duplicating matrices when a WVM floating group becomes discontiguous under integer mode-key ordering. Provider preparation expands only the matrices its native kernel requires, one family at a time, and releases each source family immediately. This setup policy is required for the issue #19 large-workload capacity preflight and remains outside timed execution.
