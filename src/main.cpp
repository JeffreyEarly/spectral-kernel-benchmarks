...
void usage(std::ostream& output) {
    output << "Usage:\n"
           ...
           << "  skbench run [--kernel fft|pruned-horizontal|vertical-gemm|ordering-packing|spectral-boundary|spectral-pipeline|production-lifetime-flux|constant-stratification-vertical|constant-stratification-flux|dealiased-convolution|vertically-batched-advection] [--profile NAME] [--providers both|fftw] [--workers N]\n"
           << "              [--spectral-flux-fixture PREPARED.bin]\n"
           << "              [--constant-stratification-flux-fixture PREPARED.bin]\n"
    ...
    try {
        ...
        if (command == "run") {
            ...
            for (int index = 2; index < argc; ++index) {
                ...
                else if (key == "--constant-stratification-flux-fixture") options.constantStratificationFluxFixture = requireValue(argc, argv, index);
